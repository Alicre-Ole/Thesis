...
<div style="background-color: #F2F2F2;">
    ...
    <div class="row adminNavigations">
        ...
        <div class="adminNavs col-md-2" style="background-color:#9DC3E6; border-color:gold; border-width:3px;">
                <?php
                    include_once('utilities/dbconnect.php');
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-7"><br>
                <h2 class="rounded adminHead">1.) Foster Care Applicants</h2>
                <div class="table-responsive">
                    <table class="table table-hover" style="background-color:#F2F2F2; font-size:15px;">
                        ...
                        <tbody>
                                <?php
                                    include_once('utilities/dbconnect.php');
                                    $sqlApply = mysqli_query($conn,"SELECT * FROM foster WHERE status = 0");
                            ...
                            <tr>
                                ...
                                <td><?= $cell; ?></td>
                                <td class="text-right">
                                <a href="viewFF.php?id=<?= $id; ?>&pID=<?= $pID; ?>" class="actions mr-2"><i class="fas fa-expand text-primary" style=" font-size:15px;" data-toggle="tooltip" data-placement="top" title="View Details"></i></a>
                                <a href="processes/appFF.php?id=<?= $id; ?>" class="actions mr-2"><i class="fas fa-check text-success" style=" font-size:15px;" data-toggle="tooltip" data-placement="top" title="Approve Application"></i></a>
                                <a href="processes/rejFF.php?id=<?= $id; ?>" onclick="return confirm('Are you Sure to Reject this Application?')" class="actions mr-2" data-toggle="tooltip" data-placement="top" title="Delete Applicant"><i class="fas fa-times text-danger" style=" font-size:15px;"></i></a>
                                </td>	
                            </tr>
                        </tbody>
                            <?php
                        ...
                        <tbody>
                                <?php
                                    include_once('utilities/dbconnect.php'); // Using database connection file here
                                        $sqlFoster = mysqli_query($conn,"SELECT * from foster WHERE approved IS NULL AND status = '1'");
                                        while($row = mysqli_fetch_array($sqlFoster))
                                        {
                                            $id = $row['id'];
                                            $pID = $row['pID'];
                                            $fname = $row['fName'];
                            ...
                            <tr>
                                <td><?= $id; ?></td>
                                <td><?= $lname; ?>, <?= $fname; ?> <?= $mi; ?>.</td>
                                <td><?= $pName; ?></td>
                                <td class="text-right">
                                <a href="viewFF_signing.php?id=<?= $id; ?>&pID=<?= $pID; ?>" class="actions mr-2"><i class="fas fa-expand text-primary" style=" font-size:15px;" data-toggle="tooltip" data-placement="top" title="View Details"></i></a>
                                <a href="processes/emailFF_signing.php?pID=<?= $pID; ?>&id=<?= $id; ?>" class="actions mr-2"><i class="fas fa-envelope text-success" style=" font-size:15px;"></i></a>
                                <a href="processes/rejFF.php?pID=<?= $pID; ?>&id=<?= $id; ?>" onclick="return confirm('Are you Sure to Cancel this Application? This will also be deleted from the application list.')" class="actions mr-2"><i class="fas fa-trash text-danger" style=" font-size:15px;"></i></a>
                                </td>	
                            </tr>
                        </tbody>
                            <?php
